...
An end-to-end **Artificial Intelligence and Machine Learning** project for cybersecurity network-traffic classification and anomaly detection.
...
*   Classify network flows into different traffic and attack categories.
    
*   Detect unusual network flows using unsupervised anomaly detection.
    
*   Identify important network features influencing model predictions.
    
*   Compare supervised learning models using appropriate evaluation metrics.
    
*   Analyse challenges associated with minority attack classes and overlapping traffic patterns.
    

The project uses a cyber network traffic dataset containing **21,185 network flows and 84 original attributes**. The target variable is the Label column, containing **10 traffic classes**.
...
ItemDetails**Course**IT7009 – Artificial Intelligence**Assessment**Practical Project – Group**Institution**Bahrain Polytechnic**Project Area**Artificial Intelligence & Cybersecurity**Programming Language**Python**Development Environment**Google Colab / Jupyter Notebook**Main ML Library**Scikit-learn**Data Processing**Pandas, NumPy**Visualisation**Matplotlib
...
*   Remove identifier fields unsuitable for machine learning.
...
*   Analyse anomaly distribution across traffic classes.
...
Each row represents a **network flow** containing statistical information about network communication.
...
Worms                          246
```
Several major classes contain 3,000 samples, while **Worms, Analysis, and Backdoor** contain substantially fewer samples.

Because of this class imbalance, **macro-averaged Precision, Recall, and F1-score** were used so that minority classes receive equal consideration during evaluation.
...
Before training the models, the dataset was checked for common data-quality issues.
...
Whitespace was removed from Label values to ensure consistency.
...
Final data-quality checks confirmed:
...
Feature engineering was performed to improve the representation of network traffic.

### Timestamp Features

Three numeric features were extracted from the timestamp:
...
Feature scaling was used because the network-traffic features have different numerical ranges.

StandardScaler was implemented inside **Scikit-learn pipelines**.

Scaling was particularly important for KNN because KNN is a distance-based algorithm.
...
*   Which traffic classes are correctly classified.
    
*   Which classes are frequently confused.
    
*   Which minority classes are difficult to classify.
...
This demonstrates an important challenge when applying machine learning to cybersecurity datasets:
...
This threshold was used as a practical decision threshold for identifying the most unusual network flows.
...
1.  **Coefficient-based global feature importance**
...
|
├── code/
|    └── AI Cyber Network Classification.ipynb
|
├── report/
|      └── AI project Report.docx
|
└── README.md
```
Code Output Files
...
File                                        Description   
supervised\_model\_comparison.csv           Comparison of supervised model performance
svm\_global\_feature\_importance.csv        Global Linear SVM coefficient-based feature importance
svm\_per\_class\_report.csv                 Linear SVM classification results for individual classes
svm\_permutation\_importance.csv            Permutation importance results
knn\_per\_class\_report.csv                 KNN classification results for individual classes
top200\_anomalies.csv                       Highest-ranked anomalous network flows
anomaly\_label\_distribution\_pct.csv       Distribution of labels among flagged anomalies
anomaly\_rate\_by\_label.csv                Anomaly rate calculated for each traffic class
...
code/AI Cyber Network Classification.ipynb
...
**IT7009 – Artificial Intelligence****Bahrain Polytechnic**

#   Group Member
* 1   Mohammed Alasfoor
* 2   Redha Mossa
* 3   Abdulelah Slais
* 4   Abdulrahman Altairey
* 5   Khalid Alzayed
* 6   Yousif Abdulsalam
...
It demonstrates the application of supervised and unsupervised machine-learning techniques to support:
